...
async def async_setup():
    ...
    admin_tracker = UsageTracker(
        user_id=first_admin,
        username='admin',
        chat_id=first_admin,
        default_max_tokens=default_max_tokens,
        are_functions_available=are_functions_available
    )
    ...
    plugin_manager = await PluginManager.create(config=plugin_config)
...
def main():
    # Run async setup, then start telegram_bot.run() synchronously outside any event loop
    telegram_bot = asyncio.run(async_setup())

    # Create a new event loop and set it as current event loop in this thread
