...
class UsageTracker:
    ...
    def __init__(self, user_id, username, chat_id, default_max_tokens=None, are_functions_available=None, logs_dir="user_logs"):
        ...
        self.openai_config = {
            ...
            'flux_api_key': os.environ.get('FLUX_API_KEY', None),
            'flux_base_url': os.environ.get('FLUX_BASE_URL', 'https://api.together.xyz/v1/images/generations'),
            'flux_image_model': os.environ.get('FLUX_IMAGE_MODEL', 'black-forest-labs/FLUX.1-schnell-Free'),
            # Proxy Settings
            ...
            'kokoro_tts_model': os.environ.get('KOKORO_TTS_MODEL', 'kokoro-tts'),
    ...
    def do_vision_conversations(self,update_value=None,chat_id=None):
        ...
        """
        if update_value:
            self.usage['vision_conversations'][chat_id].append(update_value)
            with open(self.user_file, "w") as outfile:
                json.dump(self.usage, outfile, indent=4)
            return
    ...
    def do_last_updated(self,update_value=None,chat_id=None):
        ...
        """
        if update_value:
            self.usage['last_updated'][chat_id].append(update_value)
            with open(self.user_file, "w") as outfile:
                json.dump(self.usage, outfile, indent=4)
            return
    ...
    def add_kokoro_tts_request(self, text_length, kokoro_model, kokoro_prices="0.015"):
        """Adds requested KokoroTTS text length to usage history and updates current cost.
        :param text_length: Length of text converted to speech
        :param kokoro_model: The KokoroTTS model used
        :param kokoro_prices: Price per 1K characters, defaults to 0.015
        """
        price = float(kokoro_prices)
        today = date.today()
        tts_price = round(text_length * price / 1000, 2)
        self.add_current_costs(float(tts_price))
        ...
        if kokoro_model not in self.usage['usage_history'][str(today)]['kokoro_tts_characters']:
            self.usage['usage_history'][str(today)]['kokoro_tts_characters'][kokoro_model] = 0

        # Update usage_history
        self.usage["usage_history"][str(today)]["kokoro_tts_characters"][kokoro_model] += text_length

        # Write updated token usage to user file
        with open(self.user_file, "w") as outfile:
            json.dump(self.usage, outfile, indent=4)

    def get_current_kokoro_tts_usage(self):
        """Get length of speech generated by KokoroTTS for today and this month.
        :return: total amount of characters converted to speech per day and per month
        """
        today = date.today()
        characters_day = 0
        if 'kokoro_tts_characters' in self.usage["usage_history"][str(today)]:
            for model_chars in self.usage["usage_history"][str(today)]["kokoro_tts_characters"].values():
                characters_day += model_chars

        month = str(today)[:7]  # year-month as string
        characters_month = 0
        for day, data in self.usage["usage_history"].items():
            if day.startswith(month) and 'kokoro_tts_characters' in data:
                for model_chars in data['kokoro_tts_characters'].values():
                    characters_month += model_chars
